    ...
    try {
        ...
        dispatch(
            applicationSlice.actions.failureForAllApplications(
                error?.response?.data?.message || error.message || 'Failed to fetch employer applications'
            )
        );
    }
};
...
export const fetchJobSeekerApplications = () => async (dispatch) => {
    ...
    try {
      const response = await axios.get(
        ...
        {
          withCredentials: true, // Ensures that cookies (like session or auth tokens) are sent with the request
        }
      );
      console.log('Fetched job seeker applications:', response.data.applications); // Log here
      dispatch(
        applicationSlice.actions.successForMyApplications(
          response.data.applications
        )
      );
      dispatch(applicationSlice.actions.clearAllErrors()); // Clear errors if there were any previously
    } catch (error) {
      console.error('Error fetching applications:', error);
      dispatch(
        applicationSlice.actions.failureForMyApplications(
          error?.response?.data?.message || error.message || 'Failed to fetch job seeker applications'
        )
      );
    }
  };
...
export const postApplication = (data, jobId) => async (dispatch) => {
    ...
    try {
        ...
        const response = await axios.post(
            ...
            data,  // This is now a FormData object
            {
                withCredentials: true,
        ...
        dispatch(
            applicationSlice.actions.failureForPostApplication(errorMessage)
        );
    }
};
    ...
    try {
        ...
        dispatch(
            applicationSlice.actions.failureForDeleteApplication(
                error?.response?.data?.message || error.message || 'Failed to delete application'
            )
        );
    }
};
